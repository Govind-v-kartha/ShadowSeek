...
from user_scanner.cli.display import (
    display_final_merged_results,
...
def export_single_target_results(results, target, format_type="pdf", is_email=False):
    fmt = (format_type or "pdf").lower().strip()
    if fmt not in ("pdf", "json", "csv"):
        fmt = "pdf"
    clean_target = "".join(c for c in target if c.isalnum() or c in ("-", "_", "@", "."))
    out_file = f"{clean_target}_report.{fmt}"
    ...
    if fmt == "pdf":
        ...
        try:
            ...
            with open(out_file, "wb") as f:
                f.write(pdf_bytes)
            print(f"\n{G}[+] PDF report saved to {out_file}{X}")
        except Exception as e:
            print(f"\n{R}[✘] Failed to generate PDF report: {e}{X}")
    elif fmt == "json":
        data = formatter.get_json_data(results)
        with open(out_file, "w", encoding="utf-8") as f:
            json.dump(data, f, indent=2, ensure_ascii=False)
        print(f"\n{G}[+] JSON results saved to {out_file}{X}")
    elif fmt == "csv":
        content_csv = formatter.into_csv(results, include_header=True)
        with open(out_file, "w", encoding="utf-8") as f:
            f.write(content_csv)
        print(f"\n{G}[+] CSV results saved to {out_file}{X}")
...
def interactive_session():
    ...
    while True:
        ...
        try:
            line = input(f"\n{Fore.CYAN}Shadowseek{target_hint} > {Style.RESET_ALL}").strip()
        except (KeyboardInterrupt, EOFError):
            if session_targets_results:
                display_final_merged_results(session_targets_results)
            print(f"\n{Fore.YELLOW}Exiting Shadowseek... Goodbye!{Style.RESET_ALL}")
            break
        ...
        if cmd in ("cats", "list", "4"):
            cats_user = load_categories(is_email=False)
            cats_email = load_categories(is_email=True)
            print(f"\n{Fore.CYAN}Username Categories ({len(cats_user)}):{Style.RESET_ALL} {', '.join(sorted(cats_user.keys()))}")
            print(f"{Fore.CYAN}Email Categories ({len(cats_email)}):{Style.RESET_ALL} {', '.join(sorted(cats_email.keys()))}")
            continue

        if cmd in ("summary", "results", "final", "all", "5"):
            if not session_targets_results:
                print(f"{Fore.YELLOW}[!] No scan results yet. Perform a scan first.{Style.RESET_ALL}")
            else:
                display_final_merged_results(session_targets_results)
            continue
        ...
        if cmd in ("loud", "2"):
            if not last_target:
                print(f"{Fore.YELLOW}[!] No target has been scanned yet. Please enter a username or email first.{Style.RESET_ALL}")
                continue
            skipped_sites = [r.site_name for r in last_results if r.status == Status.SKIPPED and r.site_name]
            if not skipped_sites:
                print(f"{Fore.GREEN}[i] No skipped loud modules to scan for {last_target}.{Style.RESET_ALL}")
                continue
            print(f"\n{Fore.CYAN}[*] Scanning {len(skipped_sites)} loud modules for: {last_target} ...{Style.RESET_ALL}")
            config = ScanConfig(allow_loud=True, side_by_side=True)
            modules_to_run = []
            for s in skipped_sites:
                found = find_module(s, is_email=last_is_email)
                if found:
            ...
            if modules_to_run:
                ...
                for r in last_results:
                    if r.status == Status.SKIPPED and r.site_name and r.site_name.lower() in loud_map:
                        new_results.append(loud_map[r.site_name.lower()])
                    else:
                        new_results.append(r)
                last_results = new_results
                session_targets_results[last_target] = list(last_results)
            display_scan_results(last_results, config, target=last_target, is_email=last_is_email)
            display_final_merged_results(session_targets_results)
            continue

        if cmd.startswith("export") or cmd == "3":
            if not last_results:
                print(f"{Fore.YELLOW}[!] No scan results to export yet. Perform a scan first.{Style.RESET_ALL}")
                continue
            parts = line.split()
        ...
        try:
            if is_email:
                last_results = run_email_full_batch(target, config)
            else:
                last_results = run_user_full(target, config)
            last_target = target
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="Shadowseek",
        description="Shadowseek - User & Email Scanner across platforms. Developed by govind.",
        epilog="Developed by govind",
    )
    ...
    if args.version:
        ...
        print(f"Shadowseek current version -> {G}{version}{X}")
        print(f"Creator -> {C}govind{X}")
        sys.exit(0)
    ...
    if args.interactive or not (args.username or args.email or args.username_file or args.email_file):
        if args.interactive or (sys.stdin and sys.stdin.isatty()):
            interactive_session()
            return
        else:
            parser.print_help()
    ...
    side_by_side = not show_all and not args.all
    config = ScanConfig(
    ...
    for i, target in enumerate(targets):
        ...
        if config.side_by_side:
            display_scan_results(target_results, config, target=target, is_email=is_email)
    ...
    if config.side_by_side and results:
        target_to_results = {}
        for r in results:
            t = getattr(r, "seed_target", None)
            if not t:
            ...
            if t not in target_to_results:
                target_to_results[t] = []
            target_to_results[t].append(r)
        display_final_merged_results(target_to_results)
    elif not config.side_by_side:
